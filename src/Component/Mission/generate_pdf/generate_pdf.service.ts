...
export class GeneratePdfService {
  ...
  async generatAmCoverSheet(id: number, writing: boolean) {
    ...
    try {
      ...
      for (const stdBarcode of dbResult) {
        var name: String = stdBarcode.student.First_Name + ' ' + stdBarcode.student.Second_Name + ' ' + stdBarcode.student.Third_Name;
        this.ComponantServices.generateHeaderAm(doc); // Invoke `generateHeader` function.
        this.ComponantServices.generateBody(doc,
          stdBarcode.student.grades.Name,
          stdBarcode.exam_mission.subjects.Name,
          exammission.duration + '',
          exammission.Month.split(' ')[1] + ' ' + exammission.Year, writing,
        );
        this.ComponantServices.generateTableAm(doc, 10);

        // var writing = CreatePdfDto.Writing == 1 ? true : false;

        var examVersion = exammission.pdf_V2 != null ? stdBarcode.Exam_Version : undefined;
  ...
  async generatIBCoverSheets(id: number,) {
    ...
    try {
      ...
      for (const stdBarcode of dbResult) {
        ...
        this.ComponantServices.generateBody(doc, stdBarcode.student.grades.Name, stdBarcode.exam_mission.subjects.Name,
          exammission.duration + '', exammission.Month.split(' ')[1] + ' ' + exammission.Year, false);
        this.ComponantServices.generateTableAm(doc, 10);
        // var writing = CreatePdfDto.Writing == 1 ? true : false;
        var examVersion = exammission.pdf_V2 != null ? stdBarcode.Exam_Version : undefined;

        await this.ComponantServices.generateFooterBr(doc,
          name,
          // student class must be create relastions
          stdBarcode.student.school_class.Name + '',
          exammission.Month + ' ' + exammission.Year
  ...
  async generatBrCoverSheet(id: number, writing: boolean) {
    ...
    try {
      ...
      for (const stdBarcode of dbResult) {
        ...
        this.ComponantServices.generateBody(doc, stdBarcode.student.grades.Name, stdBarcode.exam_mission.subjects.Name,
          exammission.duration + '', exammission.Month.split(' ')[1] + ' ' + exammission.Year, writing);
  ...
  async generatSeatNumber(id: number, gradeId: number) {
    ...
    try {
      ...
      for (const seat of dbResult) {
        this.ComponantServices.generateSeatNumber(doc, seat.student.First_Name + ' ' + seat.student.Second_Name, seat.student.Third_Name, seat.exam_room.Name, seat.Seat_Number, seat.grades.Name, seat.student.Religion, seat.student.Second_Lang);
      }
  ...
  async generatAttendance(RoomId: number, path: String) {
    try {
      ...
      doc.image('assetss/nis_logo.png', 50, 10, { width: 40 }).
        font('Times-Bold').fontSize(16);
      doc
      ...
      doc.lineWidth(7.5)
        ...
        .stroke();
      doc.lineWidth(1)
        .fillAndStroke("#000000").font('Times-Bold').fontSize(12)
        .text('Grade', x + 5, y + 3)
      ...
      for (const seat of StudentsInRoom.student_seat_numnbers) {
        ...
        doc
          ...
          .text(seat.student.First_Name + ' ' + seat.student.Second_Name + ' ' + seat.student.Third_Name, 85, y + 5)
        ...
        doc.text(seat.student.school_class.Name, 570, y + 5)
          .moveTo(x, y).
          lineTo(x + 590, y).stroke();
      }
      ...
      doc.moveTo(x, y + 15)
        .lineTo(x + 590, y + 15).stroke();
      z = z + 2;
      y = 50;
      x = 10;

      doc.moveTo(x, y + 20).
        lineTo(x, (z * 15) + y + 5).stroke();
      doc.moveTo(80, y + 20).
        lineTo(80, (z * 15) + y + 5).stroke();
      doc.moveTo(310, y + 20).
        lineTo(310, (z * 15) + y + 5).stroke();
      doc.moveTo(400, y + 20).
        lineTo(400, (z * 15) + y + 5).stroke();
      doc.moveTo(450, y + 35).
        lineTo(450, (z * 15) + y + 5).stroke();
      doc.moveTo(500, y + 35).
        lineTo(500, (z * 15) + y + 5).stroke();
      doc.moveTo(550, y + 20).
        lineTo(550, (z * 15) + y + 5).stroke();
      doc.moveTo(600, y + 20).
        lineTo(600, (z * 15) + y + 5).stroke();

      doc.font('Times-Bold')
        .fontSize(10)
        .text("Students Count : " + StudentsInRoom.student_seat_numnbers.length, x, (z * 15) + y + 20).
        text("French", x + 450, (z * 15) + y + 20).
        text("German", x + 500, (z * 15) + y + 20)
        ;

      doc.font('Times-Bold').fontSize(10).
        text("Christian Students Count : " + chrestienLength, x, (z * 15) + y + 40).
  ...
  async generateEnglishWriting(missionId: number,) {
    ...
    try {
      ...
      for (const stdBarcode of dbResult) {
        ...
        // }
        var name: String = stdBarcode.student.First_Name + ' ' + stdBarcode.student.Second_Name + ' ' + stdBarcode.student.Third_Name;
        this.ComponantServices.generateEnglishWritingHeader(doc, stdBarcode.student.grades.Name, stdBarcode.Exam_Version);
        this.ComponantServices.generateEnglishWritingBody(doc)
        await this.ComponantServices.generataEnglishWritingFooter(doc, stdBarcode.student.school_class.Name, exammission.Month, stdBarcode.Exam_Version, name, stdBarcode.Barcode);
        // if (doc.bufferedPageRange().start != dbResult.length - 1)
        doc.addPage();
        doc.image('assetss/waexam/AMWAPAGE2.png', 0, 0, { width: 640, height: 1000, fit: [640, 1000], align: 'center' })
          .addPage();
      }
  ...
  async generateEnglishSocialStudies(missionId: number,) {
    ...
    try {
      ...
      for (const stdBarcode of dbResult) {
        var name: String = stdBarcode.student.First_Name + ' ' + stdBarcode.student.Second_Name + ' ' + stdBarcode.student.Third_Name;
        this.ComponantServices.generateEnglishSocialStudiesHeader(doc, stdBarcode.student.grades.Name, stdBarcode.Exam_Version, stdBarcode.Barcode);
        this.ComponantServices.generateEnglishSocialStudiesBody(doc)
        await this.ComponantServices.generataEnglishWritingFooter(doc, stdBarcode.student.school_class.Name, exammission.Month, stdBarcode.Exam_Version, name, stdBarcode.Barcode);
        // if (doc.bufferedPageRange().start != dbResult.length - 1)
        doc.addPage();
        doc.image('assetss/waexam/ENSSWAPAGE2.png', 0, 0, { width: 610, height: 1000, fit: [610, 1000], align: 'center' })
          .addPage();
  ...
  async generateArabicWriting(missionId: number,) {
    ...
    try {
      ...
      for (const stdBarcode of dbResult) {
        var name: String = stdBarcode.student.First_Name + ' ' + stdBarcode.student.Second_Name + ' ' + stdBarcode.student.Third_Name;
        // doc.rtl(true);
        this.ComponantServices.generateArabicWritingHeader(doc, stdBarcode.student.grades.Name, stdBarcode.Exam_Version, stdBarcode.Barcode);
        this.ComponantServices.generateArabicWritingBody(doc)
        await this.ComponantServices.generataArabicWritingFooter(doc, stdBarcode.student.school_class.Name, exammission.Month, stdBarcode.Exam_Version, name, stdBarcode.Barcode);
        // if (doc.bufferedPageRange().start != dbResult.length - 1)
        doc.addPage();
        doc.image('assetss/waexam/ARWAPAGE2.jpg', 0, 0, { width: 610, height: 1000, fit: [610, 1000], align: 'center' })
          .addPage();
      }
